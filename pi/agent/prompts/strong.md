...
8. Do not use the Codex CLI harness. "Scout" means the `openrouter/deepseek/deepseek-v4-flash` model through the Pi harness; the explicit `openrouter/` prefix is required so the harness does not resolve the native DeepSeek provider. "Sol" means the `openai-codex/gpt-5.6-sol` model through the Pi harness. "Fable" means the Claude Code harness using the logged-in Claude subscription.
